...
def set_slide_size(width_in, height_in, margin_in=None, scale_fonts=False):
...
SZ.update({
    # 헤더 3요소 — 사용자 지정 표준 (PAGE_HEADER 기본값)
    "eyebrow":      12,   # 섹션 마커 대문자 라벨
    "page_label":   16,   # ★ 좌상단 페이지 제목
    "pre_title":    14,   # ★ 중앙 상부 소제목
    "main_title":   22,   # ★ 중앙 상부 대제목 (볼드)

    # 헤더 legacy aliases
    "pre_headline": 14,
    "headline":     22,

    # 카드/서브 헤드라인 (페이지 헤더와 별개)
    "sub_headline": 22,   # 카드 타이틀, 섹션 부제

    # ★ 본문 3요소 — 사용자 지정 표준
    "body_emphasis": 14,  # 한 줄 강조 본문
    "body_bold":     12,  # 중요 텍스트 (볼드로 사용)
    "body":          10,  # 일반 본문

    # 본문 legacy aliases (모두 위 3요소로 수렴)
    "body_sm":      10,   # = body
    "body_reading": 14,   # = body_emphasis
    "fine":         14,   # = body_emphasis
    "caption":      10,   # = body
    "caption_sm":   10,   # = body
    "label":        12,   # = body_bold
    "micro":        10,   # = body
...
def PAGE_HEADER_LIGHT(s, *, page_title="", pre="", headline="",
    ...
    if page_title:
        T(s, Inches(ML_IN), Inches(y_title),
          ...
          page_title.upper(), sz=SZ["page_label"], b=True,
...
def PAGE_HEADER(s, *, page_title="", pre="", headline="",
    ...
    if page_title:
        T(s, Inches(ML_IN), Inches(y_title),
          Inches(CW_IN * 0.5), Inches(0.35),
          page_title, sz=SZ["page_label"], b=True,
          c=page_title_color, fn=FONT_W["bold"],
          al=PP_ALIGN.LEFT)
